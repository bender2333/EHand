...
- firmware CMake 在 VS DevCmd + Ninja 下 PASS（interface target，无产物）；FPGA smoke 结构检查 PASS，`iverilog/vvp` 仿真入口已给出但本机缺仿真器，未声明真实 PL selftest PASS。
...
- HW2（工具链/硬件）：`cli/tests/unit/test_fpga_smoke.py` 已做最小 RTL/testbench 结构检查；`python fpga/tools/run_sim.py` 需要 `iverilog/vvp`，本机未安装，未执行真实 RTL 仿真；真实 PL selftest 需硬件环境。
