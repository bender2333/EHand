...
- 验证：`pytest cli/tests/unit` 16 passed；`compileall` PASS；`generate_protocol.py --check` PASS；replay 输出 `regression_pass`；device status 无 topology/无 USB 均为 `unknown`。
- firmware CMake 在 VS DevCmd + Ninja 下 PASS（interface target，无产物）；FPGA 仿真工具未在本机 PATH 找到，未声明 HW2 PASS。
...
- HW2（工具链/硬件）：本机未找到 FPGA 仿真工具（make/yosys/svlint/slang/iverilog/verilator/vivado），未执行 RTL 仿真；真实 PL selftest 需硬件环境。
