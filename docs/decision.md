...
## D-8 — plan 与 tasks.md 的关系：引用而非重复
**Date:** 2026-06-06
**Decision:** epic plan 用 `T-NN` 编号引用 `tasks.md` 的任务定义（单一来源），不重复抄描述。
每个 plan 顶部必须有一行指针"Scope tasks 的完整定义见 tasks.md（§T-NN）"。plan 自包含的只是
tasks.md 没有的部分：验收标准 + 架构要点。已固化进 CLAUDE.md。
**Why:** 用户问"T-04 在哪"——说明 plan 里只写编号、不指明去 tasks.md 查定义时，跳转不够显式，
人和 Codex 都会卡。指针行解决可发现性，同时保持 tasks.md 为任务单一来源、避免两处同步。
